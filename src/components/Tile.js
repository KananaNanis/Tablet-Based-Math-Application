...
class Tile extends React.Component {
  ...
  render() {
    ...
    let extra_dot = null, landmark = null
    if (!query_prop('hide_dot')) {
      ...
      if (global_constant.animal_landmarks[name]
        && misc && ('undefined' !== typeof misc.landmark_index)) {
        const loc = landmark_location(name, +misc.landmark_index)
        locB = with_diameter_offset(loc, diameter, extra_scale)
        landmark = (<View style={[styles.extra_dot,
        has_dot ? { opacity: 0.5 } : {},
        {
          width: extra_scale * diameter,
          height: extra_scale * diameter,
          left: locB[0], bottom: locB[1]
        }]} />)
      }
      if (has_dot) {
        const loc2B = with_diameter_offset2(misc.extra_dot, diameter, extra_scale)
        let dot_style = {}
        if (landmark && anim_info && anim_info.hasOwnProperty('move_extra_dot')) {
          ...
          dot_style = {
            'left': this.state.dotAnim.interpolate({
              inputRange: [0, 1], outputRange: [loc2B[0], locB[0]]
            }),
            'bottom': this.state.dotAnim.interpolate({
              inputRange: [0, 1], outputRange: [loc2B[1], locB[1]]
            }),
          }
        }
        extra_dot = (<Animated.View style={[styles.extra_dot,
        {
          width: extra_scale * diameter,
          height: extra_scale * diameter,
          left: loc2B[0], bottom: loc2B[1]
          //left, bottom
        },
          dot_style]} />)
      }
