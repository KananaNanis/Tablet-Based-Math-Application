...
function apply_gen_instruction(id, inst, option_delta, option_values, correct_option_value) {
  let ok = true
  let verbose = false
      ...
      if (id.startsWith('option_')) {
        ...
        if (inst) {
          ...
          if (3 == words.length && ('<' === words[1] || '>' === words[1])) {
            ...
            if ('option' == words[0]) { // this is actually a 4-fold restriction
              for (const i = 0; i < 4; ++i) {
  ...
  for (i = 0; i < max_i; ++i) {
    ...
    for (const id of all) {
      ...
      if (!Array.isArray(inst) && 'object' === typeof inst) {
        ...
        for (var key in inst) {
          if (key.startsWith('exercise')) { // only this exercise!
            const ex = +key.substr(8)
            // let curr = query_prop('config_iteration') 
            //if (!curr) curr = 0
            if (ex == curr_exercise) {
              ok = ok && apply_gen_instruction(id, inst[key], option_delta, option_values, correct_option_value);
              if (verbose) console.log('ex', ex, 'curr', curr_exercise, 'inst', inst[key])
            }
          }
        }
      } else {
        ok = ok && apply_gen_instruction(id, c[id], option_delta, option_values, correct_option_value);
        if (verbose) console.log('id', id, 'c[id]', c[id], 'ok', ok, 'option_values', option_values)
      }
    }
