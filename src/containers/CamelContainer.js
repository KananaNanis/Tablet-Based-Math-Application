...
export function get_recent_log_entries(log, descrip) {
  ...
  for (const i = log.size - 1; i >= 0; --i) {
    if ('next_config' == log.getIn([i, 1, 1]) &&
      'start' == log.getIn([i, 1, 2])) break
    if (descrip == log.getIn([i, 1, 1]))
      log_entries.unshift(log.get(i).toJS())
  }
  return log_entries
}
...
const mapStateToProps = (state, ownProps) => {
  ...
  if (err_box.has('position')) {
    ...
    err_box_updated.position[0] -= global_screen_width
      - (width + global_constant.camels.camel_offsets[0])
      + global_constant.camels.baggage_offset
    err_box_updated.position[1] -= height + global_constant.camels.camel_offsets[1]
