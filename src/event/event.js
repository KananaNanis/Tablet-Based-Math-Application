...
function handlerDispatch(type, x, y, touchID, e) {
  ...
  */
  ...
  if ("down" === type || "start" === type) {
    ...
    mouseIsDown = true
  } else if ("up" == type || "end" == type || "cancel" == type || "cleanup" == type) {
  ...
  switch (type) {
    case "down":
    case "start": touch_dispatcher('down', x, y, touchID); break
    case "move": if (global.is_mobile || mouseIsDown) { touch_dispatcher('move', x, y, touchID); } break;
    case "up":
    case "cancel":
    case "end": touch_dispatcher('up', x, y, touchID); break
    default:
...
function store_this_touch(type, x, y, touchID) {
  ...
  } else if ("up" === type || "end" === type || "cleanup" == type || "cancel" === type) {
    if (showCircles && stored_touches[touchID] && stored_touches[touchID].circle) {
      var circle = stored_touches[touchID].circle
...
function extraTouchCleanup(e, handled = []) {
  ...
  if (Object.keys(stored_touches).length > 0) {
    ...
    var cancelled = [], numPreserved = 0
    for (var key in stored_touches) {
      if (!stored_touches.hasOwnProperty(key)) continue
      //if (!allIds.includes(key) && !handled.includes(key))
      if (!arrayContainsInteger(allIds, key) && !arrayContainsInteger(handled, key) && !stored_touches[key].preserve) {
